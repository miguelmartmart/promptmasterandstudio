...
@Database(entities = [Prompt::class], version = 5, exportSchema = false)
...
abstract class PromptRoomDatabase : RoomDatabase() {
    ...
    private class PromptDatabaseCallback(
        ...
        override fun onCreate(db: SupportSQLiteDatabase) {
            ...
            // The insertion logic is moved to onOpen to handle existing databases
        }
        ...
        override fun onOpen(db: SupportSQLiteDatabase) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                // Check if the database is empty before inserting predefined prompts
                val promptCount = dao.getAllPrompts().collect { prompts ->
                    if (prompts.isEmpty()) {
                        ...
                        predefinedPrompts.forEach { dao.insert(it) }
                        android.util.Log.d("PromptDatabaseCallback", "Initial prompts inserted onOpen")
                    } else {
                        android.util.Log.d("PromptDatabaseCallback", "Database is not empty, skipping initial insertion")
                    }
                }
            }
        }
